...
def transform_mesh(index, outputs):
    """
    Transform the mesh based on the predicted outputs.
    """
    before_meshes = []
    after_meshes = []
    for j in range(32):
        before_path = os.path.join('/data3/leics/dataset/type2_process_final/single_before',f'{index}_{j}.obj')
        after_path = os.path.join('/data3/leics/dataset/type2_process_final/single_after',f'{index}_{j}.obj')
    ...
    before_meshes.export('/data3/leics/dataset/type2_process_final/before.obj')
    after_meshes.export('/data3/leics/dataset/type2_process_final/after.obj')
...
def test_tadpm(args, config, logger):
    ...
    print(f"Test Loss: {losses.avg()[0]:.4f}")
