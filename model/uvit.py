...
class UViT(nn.Module):
    def __init__(self, img_size=6, patch_size=2, in_chans=16, embed_dim=18, depth=12, num_heads=6, mlp_ratio=4.,
                 qkv_bias=False, qk_scale=None, norm_layer=nn.LayerNorm, mlp_time_embed=False, use_checkpoint=False,
                 clip_dim=1840, num_clip_token=16, conv=True, skip=True, use_ae=False):
        super().__init__()
        ...
        self.pos_embed = nn.Parameter(torch.zeros(1, 32, 18))
        ...
        # self.final_layer = nn.Conv1d(self.in_chans*2, self.in_chans, 3, padding=1) if conv else nn.Identity()
        self.final_layer = nn.LayerNorm(6)
    ...
    def forward(self, x, timesteps, context):
        ...
        '''
        x = x.float()
        # x = self.patch_embed(x) #[bs,3,768]
        # print(x.shape)
        B, L, D = x.shape

        time_token = self.time_embed(timestep_embedding(timesteps, 6))
        time_token = time_token.unsqueeze(dim=1).repeat(1,32,1)
        if self.use_ae:
