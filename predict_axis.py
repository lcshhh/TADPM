...
from dataset.dataset import AxisTestDataset
...
def test(net, names, optimizer, scheduler, test_dataset, epoch, args, autoencoder=None):
    ...
    for it, (feats_patch, center_patch, coordinate_patch, face_patch, np_Fs, index) in enumerate(
            test_dataset):
        ...
        with torch.no_grad():
            axis = torch.zeros(index.shape[0],32,6)
            for i in range(32):
                prediction = net(faces[:,i], feats[:,i], centers[:,i], Fs[:,i], cordinates[:,i])
                predicted_centers, normal1,normal2 = get_center_and_axis(prediction)
                axis[:,i] = torch.cat([normal1,normal2],dim=-1)
            for i in range(index.shape[0]):
                np.save(os.path.join('/data3/leics/dataset/mesh/axis_output',f'{index[i]}.npy'),axis[i].cpu().numpy())
            progress_bar(it, len(test_dataset))
...
if __name__ == '__main__':
    ...
    test_dataset = AxisTestDataset(dataroot,False)
