...
def test(net, names, optimizer, scheduler, test_dataset, epoch, args, autoencoder=None):
    ...
    for it, (feats_patch, center_patch, coordinate_patch, face_patch, np_Fs, axis, mesh_path) in enumerate(
        ...
        with torch.no_grad():
            ...
            axis = torch.cat([normal1[0],normal2[0]])
            mesh = trimesh.load_mesh(mesh_path[0])
            mesh.export('/data3/leics/dataset/mesh/test.obj')
            np.save('/data3/leics/dataset/mesh/test.npy',axis.cpu().numpy())
            exit()
